<?php

namespace App\Filament\Exports;

use Filament\Actions\Exports\Exporter;
use App\Models\ManagementSDM\Recruitment;
use Filament\Actions\Exports\ExportColumn;
use Filament\Actions\Exports\Models\Export;

class RecruitmentExporter extends Exporter
{
    protected static ?string $model = Recruitment::class;

    public static function getColumns(): array
    {
        return [
            ExportColumn::make('id')
                ->label('ID'),
            ExportColumn::make('job_title')
                ->label('Job Title'),
            ExportColumn::make('job_description')
                ->label('Job Description'),
            ExportColumn::make('employment_type')
                ->label('Employment Type'),
            ExportColumn::make('location')
                ->label('Location'),
            ExportColumn::make('posted_date')
                ->label('Posted Date'),
            ExportColumn::make('closing_date')
                ->label('Closing Date'),
            ExportColumn::make('status')
                ->label('Status'),
            ExportColumn::make('created_at')
                ->label('Created At'),
            ExportColumn::make('updated_at')
                ->label('Updated At'),
        ];
    }

    public static function getCompletedNotificationBody(Export $export): string
    {
        $body = 'Your recruitment export has completed and ' . number_format($export->successful_rows) . ' ' . str('row')->plural($export->successful_rows) . ' exported.';

        if ($failedRowsCount = $export->getFailedRowsCount()) {
            $body .= ' ' . number_format($failedRowsCount) . ' ' . str('row')->plural($failedRowsCount) . ' failed to export.';
        }

        return $body;
    }
}
