<div>
             ...
             <div class="grid grid-cols-1 gap-6 mb-6">
                 <div>
                    ...
                    <div class="bg-gray-100 p-4 rounded-lg">
                        ...
                        <p class="mb-2"><strong>Kantor : </strong>{{ $schedule->company->name }}</p>
                        <p class="mb-2"><strong>Shift : </strong>{{ $schedule->shift->name }}
                            ({{ \Carbon\Carbon::parse($schedule->shift->start_time)->format('H:i') }} -
                            {{ \Carbon\Carbon::parse($schedule->shift->end_time)->format('H:i') }}) WIB</p>
                        @if ($schedule->is_wfa)
    ...
    <script>
        ...
        let lat;
        let lng;
        const company = [{{ $schedule->company->latitude }}, {{ $schedule->company->longitude }}];
        const radius = {{ $schedule->company->radius }};
        let component;
        let marker;
        document.addEventListener('livewire:initialized', function() {
            component = @this;
            map = L.map('map').setView([{{ $schedule->company->latitude }}, {{ $schedule->company->longitude }}],
                15);
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);

            const circle = L.circle(company, {
                color: 'red',
                fillColor: '#f03',
                fillOpacity: 0.5,
                radius: radius
        ...
        function tagLocation() {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(function(position) {
                    ...
                    if (isWithinRadius(lat, lng, company, radius)) {
